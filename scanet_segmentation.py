...
def visualization(im, parsing_anno, stride=1, save_im=False, save_path = None, num_of_class = 20):
    ...
    # BGRA
    part_colors = [[255, 0, 0, 150], [255, 85, 0, 150], [255, 170, 0, 150],
                   [255, 0, 85, 150], [255, 0, 170, 150],
                   [0, 255, 0, 150], [85, 255, 0, 150], [170, 255, 0, 150],
                   [0, 255, 85, 150], [0, 255, 170, 150],
                   [0, 0, 255, 150], [85, 0, 255, 150], [170, 0, 255, 150],
                   [0, 85, 255, 150], [0, 170, 255, 150],
                   [255, 255, 0, 150], [255, 255, 85, 150], [255, 255, 85, 150],
                   [255, 0, 255, 150], [255, 85, 255, 150], [255, 170, 255, 150],
                   [0, 255, 255, 150], [85, 255, 255, 150], [170, 255, 255, 150]]
    ...
    vis_im = cv2.addWeighted(cv2.cvtColor(vis_im, cv2.COLOR_BGR2BGRA), 0.6, vis_parsing_anno_color, 0.4, 0,dtype=cv2.CV_32F) 
...
def evaluate_scanet(respth='./image_folder/testing_scanet/', dspth='./image_folder/test-img', cp='/best_256.onnx'):

    # Load pretrained model
    ort_session = onnxruntime.InferenceSession("./model_pth/"+cp, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])

    interp = torch.nn.Upsample(size=(256, 256), mode='bilinear', align_corners=True)

    for image_path in os.listdir(dspth):
        # Preprocess Image
        image = cv2.imread(dspth + image_path)
        ...
        color_img = visualization(image, parsing_preds, save_path = osp.join(respth, image_path))
...
if __name__ == "__main__":
    evaluate_scanet(respth = './image_folder/testing_scanet/', dspth = './image_folder/test-img/', cp='best_256.onnx')